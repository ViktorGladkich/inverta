    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl border border-neutral-100 p-1 flex overflow-hidden ring-1 ring-black/5">
        <div className="grid grid-cols-12 w-full min-h-[320px]">
          {/* Linker Bereich: Hauptkategorie Infos */}
          <div className="col-span-4 bg-white p-8 rounded-l-xl flex flex-col justify-between border-r border-neutral-100">
            ...
            <Link
              href={item.href}
              title={`Zur Übersichtsseite: ${item.title}`}
          ...
          {/* Rechter Bereich: Spezifische SEO-verlinkte Leistungen */}
          <ul className="col-span-8 p-8 grid grid-cols-2 gap-4 bg-[#f5f5f5] rounded-r-xl">
            {item.items.map((subItem) => (
              <li key={subItem.title}>
                <Link
                  href={subItem.href}
                  title={`Mehr erfahren über ${subItem.title}: ${subItem.desc}`}
                  className="group flex flex-col justify-between h-full p-5 rounded-[20px] bg-[#f5f5f5] transition-all duration-300 hover:scale-[1.02] shadow-[0px_0.706592px_0.706592px_-0.666667px_rgba(0,0,0,0.08),0px_1.80656px_1.80656px_-1.33333px_rgba(0,0,0,0.08),0px_3.62176px_3.62176px_-2px_rgba(0,0,0,0.07),0px_6.8656px_6.8656px_-2.66667px_rgba(0,0,0,0.07),0px_13.6468px_13.6468px_-3.33333px_rgba(0,0,0,0.05),0px_30px_30px_-4px_rgba(0,0,0,0.02),inset_0px_3px_1px_0px_white]"
                >
